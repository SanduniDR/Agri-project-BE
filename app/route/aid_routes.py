...
# @jwt_required()
def get_fuel_aid_by_id(fuelAid_id):
    fuel_aid = Fuel.query.get(fuelAid_id)
    return fuel_schema.jsonify(fuel_aid)

@aid_routes.route('/fuel-aid', methods=['GET'])
# @jwt_required()
def get_all_fuel_aid():
    fuel_aid = Fuel.query.all()
    return fuels_schema.jsonify(fuel_aid)

@aid_routes.route('/fuel-aid/<fuelAid_id>', methods=['DELETE'])
# @jwt_required()
def delete_fuel_aid(fuelAid_id):
    fuel_aid = Fuel.query.get(fuelAid_id)
    db.session.delete(fuel_aid)
    db.session.commit()
    return jsonify({'message': 'Fuel aid deleted successfully!'})

@aid_routes.route('/fuel-aid/<fuelAid_id>', methods=['PUT'])
# @jwt_required()
def update_fuel_aid(fuelAid_id):
    fuel_aid = Fuel.query.get(fuelAid_id)

    if 'aid_id' in request.json:
        fuel_aid.aid_id = request.json['aid_id']

    if 'description' in request.json:
        fuel_aid.description = request.json['description']

    if 'reason' in request.json:
        fuel_aid.reason = request.json['reason']

    if 'fuel_type' in request.json:
        fuel_aid.fuel_type = request.json['fuel_type']
...
def  add_fuel_distribution():
    ...
    newFuelAid = Fuel(aid_id=aid_id, description=description, reason=reason, fuel_type=fuel_type)
    db.session.add(newFuelAid)
    db.session.commit()

    return jsonify({'message': 'New Fuel Aid details added successfully!'}), 200
...
def search_fuel_aid():
    ...
    if description:
        filter_conditions.append(Fuel.description.ilike(f'%{description}%'))
    if aid_id:
        filter_conditions.append(Fuel.aid_id.ilike(f'%{aid_id}%'))

    # Search for fuel aid records based on the filters
    query = Fuel.query.filter(*filter_conditions)
    pagination = query.paginate(page=page, per_page=per_page)
